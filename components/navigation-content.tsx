...
function ColorfulNavigationCard({ item }: { item: NavigationItem | NavigationSubItem }) {
  // 使用类型断言将 item 转换为 any，然后传递给 NavigationCard
  // 这样 NavigationCard 会处理它自己的类型检查
  const cardItem = item as any
  
  const isColorful = cardItem.special === 'colorful' || 
                    cardItem.colorEffect === 'rainbow' ||
                    cardItem.title?.includes('炫彩')

  return (
    <div className={isColorful ? 'colorful-item-wrapper' : ''}>
...
export function NavigationContent({ navigationData, siteData }: NavigationContentProps) {
  ...
  return (
    <div className="flex flex-col sm:flex-row min-h-screen">
      ...
      {/* 修改后的炫彩样式 - 确保边框完整显示 */}
      <style jsx global>{`
        .colorful-item-wrapper {
          position: relative;
          border-radius: 8px;
          padding: 2px; /* 添加内边距来显示边框 */
