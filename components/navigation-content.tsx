...
export function NavigationContent({ navigationData, siteData }: NavigationContentProps) {
  ...
  // 判断是否为炫彩网站的辅助函数
  const isColorfulItem = (item: NavigationItem | NavigationSubItem) => {
    // 根据特定条件判断，这里可以根据需要修改判断逻辑
    return item.special === 'colorful' || 
           item.colorEffect === 'rainbow' ||
           item.title.includes('炫彩') || // 根据标题包含"炫彩"关键词
           item.id === 'specific-colorful-id' // 根据特定ID
  }
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                          {(subCategory.items || []).map((item) => (
                            <NavigationCard 
                              key={item.id} 
                              item={item}
                              className={isColorfulItem(item) ? 'colorful-item' : ''}
                            />
                          ))}
                        </div>
                      </div>
